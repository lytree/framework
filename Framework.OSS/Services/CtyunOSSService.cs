...
using Framework.OSS.Models.Ctyun;
...
namespace Framework.OSS.Services
{
    ...
    public class CtyunOSSService : BaseOSSService, ICtyunOSSService
    {
        ...
        {
            HttpRequestHelper httpReqHelper = new HttpRequestHelper($"https://{_serviceURL}");
        ...
        public Task<bool> CreateBucketAsync(string bucketName)
        {
            ...
            // 创建存储空间。
            HttpRequestHelper httpReqHelper = new HttpRequestHelper($"https://{bucketName}.{_serviceURL}");
            string datetime = DateTime.Now.ToUniversalTime().ToString("R");
        ...
        public async Task GetObjectAsync(string bucketName, string objectName, Action<Stream> callback, CancellationToken cancellationToken = default)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                string url = $"https://{bucketName}.{_serviceURL}/{objectName}";
        ...
        public Task<bool> CopyObjectAsync(string bucketName, string objectName, string destBucketName = null, string destObjectName = null)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var response = httpClient.PutAsync($"https://{destBucketName}.{_serviceURL}/{destObjectName}", new StringContent(string.Empty));

                if (response.Result.IsSuccessStatusCode)
                {
                    response = httpClient.PutAsync($"https://{destBucketName}.{_serviceURL}/{destObjectName}?copy-source={Uri.EscapeDataString(source)}", new StringContent(string.Empty));

...
public static partial class Helper
{
    ...
    {
        return NumbericRegex().IsMatch(str);
    }
    ...
    public static bool IsMobile(this string str)
    {
        ...
        return MobileRegex().IsMatch(str);
    }
    ...
    {
        if (long.TryParse(str[..17], out long n) == false || n < Math.Pow(10, 16) || long.TryParse(str.Replace('x', '0').Replace('X', '0'), out n) == false)
        {
            return false;//数字验证
        }
        const string address = "11x22x35x44x53x12x23x36x45x54x13x31x37x46x61x14x32x41x50x62x15x33x42x51x63x21x34x43x52x64x65x71x81x82x91";
        if (!address.Contains(str[..2]))
        {
            return false;//省份验证
        }
    ...
    {
        return OnlyChineseRegex().IsMatch(strln);
    }
    ...
    {
        return NzxRegex().Match(str).Success;
    }
    ...
    {
        return SzzmChineseRegex().Match(str).Success;
    }
    ...
    {
        return SzzmRegex().Match(str).Success;
    }
    ...
    {
        return PostCodeRegex().IsMatch(str);
    }
    ...
    public static bool IsDateTime(this string str)
    {
        ...
        if (DateTimeRegex().IsMatch(str))
        {
            ...
            int iYear = Convert.ToInt32(str[..indexY]);
            int iMonth = Convert.ToInt32(str[(indexY + 1)..(indexM - indexY - 1)]);
            int iDate = Convert.ToInt32(str[(indexM + 1)..(indexD - indexM - 4)]);
            //判断月份日期
    ...
    [GeneratedRegex("^([0]|([1-9]+\\d{0,}?))(.[\\d]+)?$")]
    private static partial Regex NumberRegex();
    [GeneratedRegex("^-?\\d+$|^(-?\\d+)(\\.\\d+)?$")]
    private static partial Regex NumbericRegex();
    [GeneratedRegex(@"^(0[0-9]{2,3}\-)?([2-9][0-9]{6,7})+(\-[0-9]{1,4})?$", RegexOptions.IgnoreCase, "zh-CN")]
    private static partial Regex TelRegex();
    [GeneratedRegex(@"(^(\d{2,4}[-_－—]?)?\d{3,8}([-_－—]?\d{3,8})?([-_－—]?\d{1,7})?$)|(^0?1[35]\d{9}$)")]
    private static partial Regex PhoneRegex();
    [GeneratedRegex(@"^[+]{0,1}(\d){1,3}[ ]?([-]?((\d)|[ ]){1,12})+$")]
    private static partial Regex FaxRegex();
    [GeneratedRegex(@"^(13|14|15|16|17|18|19)\d{9}$")]
    private static partial Regex MobileRegex();
    [GeneratedRegex(@"^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$")]
    private static partial Regex EmailRegex();
    [GeneratedRegex(@"^[\u4e00-\u9fa5]+$")]
    private static partial Regex OnlyChineseRegex();
    [GeneratedRegex("^[0-9a-zA-Z_]+$")]
    private static partial Regex NzxRegex();
    [GeneratedRegex(@"^[0-9a-zA-Z\u4e00-\u9fa5]+$")]
    private static partial Regex SzzmChineseRegex();
    [GeneratedRegex(@"^[0-9a-zA-Z]+$")]
    private static partial Regex SzzmRegex();
    [GeneratedRegex(@"\d{6}")]
    private static partial Regex PostCodeRegex();
    [GeneratedRegex(@"[1-2]{1}[0-9]{3}((-|\/|\.){1}(([0]?[1-9]{1})|(1[0-2]{1}))((-|\/|\.){1}((([0]?[1-9]{1})|([1-2]{1}[0-9]{1})|(3[0-1]{1})))( (([0-1]{1}[0-9]{1})|2[0-3]{1}):([0-5]{1}[0-9]{1}):([0-5]{1}[0-9]{1})(\.[0-9]{3})?)?)?)?$")]

			...
			where T : IModbusMessage, new()
		{
			...
			do
			{
					...
					lock (_syncLock)
					{
						...
						do
						{
							...
							if (exceptionResponse != null)
							{
								...
								if (readAgain)
								{
									Logger.LogDebug($"Received ACKNOWLEDGE slave exception response, waiting {_waitToRetryMilliseconds} milliseconds and retrying to read response.");
				...
				catch (SlaveException se)
				{
					...
					Logger.LogWarning($"Received SLAVE_DEVICE_BUSY exception response, waiting {_waitToRetryMilliseconds} milliseconds and resubmitting request.");
				...
				catch (Exception e)
				{
					...
					{
						Logger.LogError($"{e.GetType().Name}, {(_retries - attempt + 1)} retries remaining - {e}");
